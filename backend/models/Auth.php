...
header("Access-Control-Allow-Credentials: true");
	...
	class Auth {
		...
		public function registeradmin($dt) {
			$payload = "";
			$remarks = "";
			$message = "";
			
			// Extracting user inputs from $dt object
			$admin_name = $dt->admin_name;
			$admin_email = $dt->admin_email;
			$admin_password = $dt->admin_password;
		
			// Encrypting password
			$encryptedPassword = $this->encrypt_password($admin_password);
			...
			$sql = "INSERT INTO admin(admin_name, admin_email, admin_password) 
					VALUES ('$admin_name', '$admin_email', '$encryptedPassword')";
		...
		public function loginadmin($dt){
			...
			// Check if the user is an admin
			$sql = "SELECT * FROM admin WHERE admin_email='$admin_email' LIMIT 1";
			$res = $this->gm->generalQuery($sql, "Incorrect username or password");
			if($res['code'] == 200) {
				...
				if($this->pword_check($admin_password, $res['data'][0]['admin_password'])) {
					$code = 200;
					$remarks = "success";
					$message = "Logged in successfully";
					$payload = array("admin_id" => $admin_id, "admin_email" => $admin_email, "admin_name" => $admin_name);
				} else {
					$payload = null; 
					$remarks = "failed"; 
					$message = "Incorrect username or password";
